...
public class PictureGameLayer extends CCLayer {
	...
	public PictureGameLayer() {
		...
		statusLabel.setPosition(CGPoint.ccp(25 * generalscalefactor,
				screenSize.height - 10 * generalscalefactor));
		...
		timerLabel.setPosition(CGPoint.ccp(screenSize.width - 25
				* generalscalefactor, screenSize.height - 10
				* generalscalefactor));
		...
		movesLabel.setPosition(CGPoint.ccp(screenSize.width - 25
				* generalscalefactor,
				timerLabel.getPosition().y - timerLabel.getContentSize().height
						* generalscalefactor - 10 * generalscalefactor
						- timerLabel.getContentSize().height
						* generalscalefactor));
		...
		CCMenuItemLabel item5 = CCMenuItemLabel.item(label, this,
				"menuCallback");
		item5.setScale(1.0f * generalscalefactor);
		CCMenu backemenu = CCMenu.menu(item5);
		backemenu.setPosition(CGPoint.make(
				screenSize.width - item5.getContentSize().width * 1.5f
						* generalscalefactor, item5.getContentSize().height
						* 1.5f * generalscalefactor));
	...
	public void generateTiles() {
		...
		// We attempt to calculate the right size for the tiles given the screen
		// size and
		...
		int useablewidth = (int) (screenSize.width - statusLabel
				.getContentSize().width * generalscalefactor);
		useableheight = (int) (screenSize.height - 40 * generalscalefactor - statusLabel
				.getContentSize().height * 1.3f * generalscalefactor);

		TILE_SQUARE_SIZE = (int) Math.min((useableheight / NUM_ROWS),
				(useablewidth / NUM_COLUMNS));
		...
		for (int j = toppoint; j > toppoint - (TILE_SQUARE_SIZE * NUM_ROWS); j -= TILE_SQUARE_SIZE) {
			for (int i = topleft; i < (topleft - 5 * generalscalefactor)
					+ (TILE_SQUARE_SIZE * NUM_COLUMNS); i += TILE_SQUARE_SIZE) {
				...
				// Add Tile number
				CCBitmapFontAtlas tileNumber = CCBitmapFontAtlas
						.bitmapFontAtlas("00", "magneto.fnt");
				tileNumber.setScale(2.0f * scalefactor);
				tileNumber.setString(nextval + ""); // add tile number to keep
													// track of it
		...
		for (float j = 0; j < NUM_ROWS; j++) {
			for (float i = 0; i < NUM_COLUMNS; i++) {
				...
				// Calculate the size of each tile by dividing the height and
				// width of our image
				// and returning the min value of both calculations
				float theblock = Math.min(mybit.getHeight() / NUM_ROWS,
						mybit.getWidth() / NUM_COLUMNS);

				// Create a new sprite using this dimension above and from a
				// given portion of the image
				CCSpriteFrame myframe = CCSpriteFrame.frame(metexture, CGRect
						.make(i * theblock, j * theblock, theblock, theblock),
						CGPoint.make(0, 0));
				tile = CCSprite.sprite(myframe);
				tile.setScale((TILE_SQUARE_SIZE / theblock)
						* (1.2f * scalefactor));

				// Assign our newly created sprite background to a node created
				// earlier.
	...
	public boolean ccTouchesBegan(MotionEvent event) {
		...
		for (int i = 1; i < (NUM_ROWS * NUM_COLUMNS); i++) {
			...
			if (spritePos.contains(location.x, location.y)) {
				...
				slideCallback(eachNode); // if yes, we pass the tile for
											// sliding.
	...
	public void slideCallback(CCNodeExt thenode) {
		...
		// Determine the position to slide the tile to .. ofcourse only if
		// theres an empty space beside it

		if ((nodePosition.x - TILE_SQUARE_SIZE) == emptyPosition.x
				&& nodePosition.y == emptyPosition.y) {
			slideTile("Left", thenode, true);
		} else if ((nodePosition.x + TILE_SQUARE_SIZE) == emptyPosition.x
				&& nodePosition.y == emptyPosition.y) {
			slideTile("Right", thenode, true);
		} else if ((nodePosition.x) == emptyPosition.x
				&& nodePosition.y == (emptyPosition.y + TILE_SQUARE_SIZE)) {
	...
	public void slideTile(String direction, CCNodeExt thenode, boolean move) {
		...
		if (move && !gameover) {
			...
			CCSequence movetileSeq = CCSequence.actions(movetile,
					CCCallFuncN.action(this, "handleWin"));
	...
	public boolean checkCorrect() {
		...
		for (float j = toppoint; j > toppoint - (TILE_SQUARE_SIZE * NUM_ROWS); j -= TILE_SQUARE_SIZE) {
			for (float i = topleft; i < (topleft - 5)
					+ (TILE_SQUARE_SIZE * NUM_COLUMNS); i += TILE_SQUARE_SIZE) {
				if (tileNode.getChildByTag(nodeindex).getPosition().x == i
						&& tileNode.getChildByTag(nodeindex).getPosition().y == j) {
					result = true;
				} else {
					return false;
				}
	...
	public void WinCallback(Object sender) {
		...
		CCBitmapFontAtlas gamemoves = CCBitmapFontAtlas.bitmapFontAtlas(
				CCFormatter.format("%02d", moves) + " Moves", "magneto.fnt");
		...
		gamemoves.setPosition(CGPoint.ccp(screenSize.width / 2.0f,
		...
		CCBitmapFontAtlas instructionFontAtlas = CCBitmapFontAtlas
				.bitmapFontAtlas("TAP Back button below to continue!",
						"magneto.fnt");
		instructionFontAtlas.setPosition(gamemoves.getPosition().x,
				-instructionFontAtlas.getContentSize().height * 0.6f
						* generalscalefactor);
		...
		instructionFontAtlas.runAction(CCSequence.actions(
				...
				CCMoveTo.action(
						0.5f,
						CGPoint.make(
								gamemoves.getPosition().x,
								gamemoves.getPosition().y - 10
										* generalscalefactor
	...
	public static void getBitMapfromCamera() {
		...
		MainActivity.app.startActivityForResult(intent,
	...
	public static void getBitmapFromAsset(String strName) throws IOException {
		AssetManager assetManager = CCDirector.sharedDirector().getActivity()
				.getAssets();
